...
// ASL Express primitives - same shapes as the other case studies so they all
// render as siblings. Accent: emerald #10B981 with #6EE7B7 bright. Pure ASCII
// source (the editor's on-save formatter truncates files with em-dashes /
// other multibyte glyphs).

export const ASL_NAV = [
  { id: 'why', label: 'Why this exists' },
  { id: 'signify', label: 'Origin: Signify' },
  { id: 'decision', label: 'The pivot' },
...
export function SideNav({ activeId = '' }) {
  return (
    <nav
      className="fixed left-0 top-12 z-40 hidden h-[calc(100vh-3rem)] w-[220px] flex-col border-r border-white/[0.06] bg-[#0B120F]/95 px-5 py-10 backdrop-blur-sm lg:flex"
            ...
            <li key={item.id}>
              <a
                ...
                className={`flex items-center gap-2 py-1.5 font-[family-name:var(--font-mono)] text-[11px] uppercase tracking-[0.14em] transition-colors ${
                  active ? 'text-[#6EE7B7]' : 'text-zinc-500 hover:text-zinc-300'
                }`}
              >
                <span
                  className={`h-1 w-1 shrink-0 rounded-full ${active ? 'bg-[#6EE7B7]' : 'bg-transparent'}`}
                  aria-hidden
                />
                {item.label}
              </a>
...
export function Highlight({ children, className = '' }) {
  return (
    <mark className={`box-decoration-clone rounded-[3px] bg-[#10B981]/25 px-1.5 py-0.5 font-medium text-white ${className}`}>
      {children}
    </mark>
  )
}
...
export function LinkButton({ href, children }) {
  return (
    <a
      ...
      className="inline-flex items-center gap-1.5 rounded-md border border-[#10B981]/40 bg-[#10B981]/[0.08] px-4 py-2 font-[family-name:var(--font-mono)] text-[11px] uppercase tracking-[0.16em] text-[#6EE7B7] transition-colors hover:border-[#10B981] hover:bg-[#10B981]/[0.16] hover:text-white"
    >
      {children}
    </a>
  )
...
export function MediaSlot({ label = 'media', note = '', aspect = '16 / 9', className = '' }) {
  return (
    <div
      className={`flex w-full flex-col items-center justify-center gap-2 rounded-xl border border-dashed border-[#10B981]/30 bg-[#10B981]/[0.04] px-6 py-10 text-center ${className}`}
      style={{ aspectRatio: aspect }}
      role="img"
      aria-label={`${label} placeholder`}
    >
      <span className="font-[family-name:var(--font-mono)] text-[10px] uppercase tracking-[0.2em] text-[#6EE7B7]">
        {label}
      </span>
...
export function Figure({ src, alt, caption, aspect = '16 / 9', position = 'center', className = '' }) {
  return (
    <figure className={className}>
      <div className="overflow-hidden rounded-xl border border-white/[0.08] bg-white/[0.02]" style={{ aspectRatio: aspect }}>
        <img src={src} alt={alt} loading="lazy" decoding="async" className="h-full w-full object-cover" style={{ objectPosition: position }} />
      </div>
      {caption ? (
