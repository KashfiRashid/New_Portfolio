...
// Market Basket Analysis primitives - same shapes as the other case studies so
// they render as siblings. Accent: amber #E8B86A with #F4D6A6 bright (the site
// monitor-glow). Pure ASCII source (the editor on-save formatter truncates
// files with em-dashes / other multibyte glyphs).

export const MB_NAV = [
  { id: 'problem', label: 'The problem' },
  { id: 'evolution', label: 'The pivot' },
  { id: 'phase-1', label: 'High-value AI' },
  { id: 'phase-2', label: 'The recommender' },
  { id: 'demo', label: 'The demo' },
  { id: 'my-role', label: 'What I did' },
...
export function SideNav({ activeId = '' }) {
  return (
    <nav
      className="fixed left-0 top-12 z-40 hidden h-[calc(100vh-3rem)] w-[220px] flex-col border-r border-white/[0.06] bg-[#0E0C0A]/95 px-5 py-10 backdrop-blur-sm lg:flex"
            ...
            <li key={item.id}>
              <a
                ...
                className={`flex items-center gap-2 py-1.5 font-[family-name:var(--font-mono)] text-[11px] uppercase tracking-[0.14em] transition-colors ${
                  active ? 'text-[#F4D6A6]' : 'text-zinc-500 hover:text-zinc-300'
                }`}
              >
                <span
                  className={`h-1 w-1 shrink-0 rounded-full ${active ? 'bg-[#F4D6A6]' : 'bg-transparent'}`}
                  aria-hidden
                />
                {item.label}
              </a>
...
export function Highlight({ children, className = '' }) {
  return (
    <mark className={`box-decoration-clone rounded-[3px] bg-[#E8B86A]/25 px-1.5 py-0.5 font-medium text-white ${className}`}>
      {children}
    </mark>
  )
}
...
export function LinkButton({ href, children }) {
  return (
    <a
      ...
      className="inline-flex items-center gap-1.5 rounded-md border border-[#E8B86A]/40 bg-[#E8B86A]/[0.08] px-4 py-2 font-[family-name:var(--font-mono)] text-[11px] uppercase tracking-[0.16em] text-[#F4D6A6] transition-colors hover:border-[#E8B86A] hover:bg-[#E8B86A]/[0.16] hover:text-white"
    >
      {children}
    </a>
  )
...
export function Stat({ value, label, sub }) {
  return (
    <div className="border border-white/[0.08] bg-white/[0.02] p-6">
      ...
      {sub ? <div className="mt-2 font-[family-name:var(--font-sans)] text-[13px] leading-relaxed text-zinc-400">{sub}</div> : null}
    </div>
  )
}
